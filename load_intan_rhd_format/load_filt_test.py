...
if __name__ == '__main__':
    #a=read_data(sys.argv[1]sampledata.rhd)
    a=read_data("D:\\emgsensor\\exp1031\\four_f_221031_155542\\four_f_221031_155542.rhd")
    b=a['amplifier_data']
    
    #np.savetxt("raw_1030test.csv", b.T, delimiter=",")
    print(b.shape)
    
    b = b[:,2000:]
    
    filted_data = np.zeros((128,1))
    
    cishu = math.floor((b.shape[1]/768))

    for i in range(cishu):
        linshi = b[:,i*768:i*768+768]
        linshi_f = guolv(linshi)
        filted_data = np.concatenate((filted_data, linshi_f), axis=1)
        
    filted_data = filted_data[:,1:]
    
    #print(linshi.shape)
    print(filted_data.shape)
    np.savetxt("filt_1031ff3.csv", filted_data.T, delimiter=",")
