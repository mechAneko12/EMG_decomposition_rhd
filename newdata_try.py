...
#W = np.load("Ica_W.npy")
#K = np.load("Ica_K.npy")
cluster_centers = np.load("cluster_centers_1031ff2.npy")
emg_mu_for_kmeans = np.load("emg_mu_for_kmeans_1031ff2.npy")
...
# load FastIca model
emg_FastICA = load("emg_FastICA_1031ff2.joblib")

# load kmeans model
_kmeans = load("emg_kmeans_1031ff2.joblib")
_kmeans.fit(emg_mu_for_kmeans[:,[1]])


# load the new data, delete this part when doing online
p = r'filt_1031ff3.csv'
...
print('start kmeans')
cishu = math.floor((data.shape[0]/768))
spike_trains_all = np.zeros((1,20))

for j in range(cishu):
    emg_raw = emg[j*768:j*768+768,:]
    ...
    for i in range(emg_mu_squared.shape[1]):
        ...
        spike_trains[:, i] = _kmeans.predict(emg_mu_squared[:, [i]])
    ...
    spike_trains_all = np.concatenate((spike_trains_all, spike_trains_processsed), axis=0)


print('end kmeans')


plotspikes(spike_trains_all,title='1031ff3_new2')

#np.savetxt("five1fle_new.csv", spike_trains, delimiter=",")
print('end try')
